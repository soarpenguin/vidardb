...
#ifndef STORAGE_VIDARDB_INCLUDE_ENV_H_
#define STORAGE_VIDARDB_INCLUDE_ENV_H_
...
class Env {
 public:
  ...
  //
  // The result of Default() belongs to vidardb and must never be deleted.
  static Env* Default();
  ...
  // used in VidarDB internally to update thread status and supports
  // GetThreadList().
...
class SequentialFile {
 public:
  SequentialFile() { }
  /********************** Shichao ************************/
  SequentialFile(const std::string& filename)
    : filename_(filename) { }
  /********************** Shichao ************************/
...
#endif  // STORAGE_VIDARDB_INCLUDE_ENV_H_
