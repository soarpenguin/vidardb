...
#ifndef STORAGE_VIDARDB_INCLUDE_DB_H_
#define STORAGE_VIDARDB_INCLUDE_DB_H_
...
class ColumnFamilyHandle {
 public:
  ...
  //
  // Note that this function is not supported in VidarDBLite.
...
class DB {
 public:
  ...
  // in vidardb::kDefaultColumnFamilyName
  //
  // Not supported in VIDARDB_LITE, in which case the function will
  // return Status::NotSupported.
  ...
  struct Properties {
    //  "vidardb.num-files-at-level<N>" - returns string containing the number
    //      of files at level <N>, where <N> is an ASCII representation of a
    //      level number (e.g., "0").
    static const std::string kNumFilesAtLevelPrefix;

    //  "vidardb.compression-ratio-at-level<N>" - returns string containing the
    ...
    //  "vidardb.stats" - returns a multi-line string containing the data
    //      described by kCFStats followed by the data described by kDBStats.
    static const std::string kStats;

    //  "vidardb.sstables" - returns a multi-line string summarizing current
    //      SST files.
    static const std::string kSSTables;

    //  "vidardb.cfstats" - returns a multi-line string with general column
    ...
    //  "vidardb.dbstats" - returns a multi-line string with general database
    //      stats, both cumulative (over the db's lifetime) and interval (since
    //      the last retrieval of kDBStats).
    static const std::string kDBStats;

    //  "vidardb.levelstats" - returns multi-line string containing the number
    //      of files per level and total size of each level (MB).
    static const std::string kLevelStats;

    //  "vidardb.num-immutable-mem-table" - returns number of immutable
    //      memtables that have not yet been flushed.
    static const std::string kNumImmutableMemTable;

    //  "vidardb.num-immutable-mem-table-flushed" - returns number of immutable
    //      memtables that have already been flushed.
    static const std::string kNumImmutableMemTableFlushed;

    //  "vidardb.mem-table-flush-pending" - returns 1 if a memtable flush is
    //      pending; otherwise, returns 0.
    static const std::string kMemTableFlushPending;

    //  "vidardb.num-running-flushes" - returns the number of currently running
    //      flushes.
    static const std::string kNumRunningFlushes;

    //  "vidardb.compaction-pending" - returns 1 if at least one compaction is
    //      pending; otherwise, returns 0.
    static const std::string kCompactionPending;

    //  "vidardb.num-running-compactions" - returns the number of currently
    //      running compactions.
    static const std::string kNumRunningCompactions;

    //  "vidardb.background-errors" - returns accumulated number of background
    //      errors.
    static const std::string kBackgroundErrors;

    //  "vidardb.cur-size-active-mem-table" - returns approximate size of active
    //      memtable (bytes).
    static const std::string kCurSizeActiveMemTable;

    //  "vidardb.cur-size-all-mem-tables" - returns approximate size of active
    //      and unflushed immutable memtables (bytes).
    static const std::string kCurSizeAllMemTables;

    //  "vidardb.size-all-mem-tables" - returns approximate size of active,
    //      unflushed immutable, and pinned immutable memtables (bytes).
    static const std::string kSizeAllMemTables;

    //  "vidardb.num-entries-active-mem-table" - returns total number of entries
    //      in the active memtable.
    static const std::string kNumEntriesActiveMemTable;

    //  "vidardb.num-entries-imm-mem-tables" - returns total number of entries
    //      in the unflushed immutable memtables.
    static const std::string kNumEntriesImmMemTables;

    //  "vidardb.num-deletes-active-mem-table" - returns total number of delete
    //      entries in the active memtable.
    static const std::string kNumDeletesActiveMemTable;

    //  "vidardb.num-deletes-imm-mem-tables" - returns total number of delete
    //      entries in the unflushed immutable memtables.
    static const std::string kNumDeletesImmMemTables;

    //  "vidardb.estimate-num-keys" - returns estimated number of total keys in
    //      the active and unflushed immutable memtables.
    static const std::string kEstimateNumKeys;

    //  "vidardb.estimate-table-readers-mem" - returns estimated memory used for
    ...
    //  "vidardb.is-file-deletions-enabled" - returns 0 if deletion of obsolete
    //      files is enabled; otherwise, returns a non-zero number.
    static const std::string kIsFileDeletionsEnabled;

    //  "vidardb.num-snapshots" - returns number of unreleased snapshots of the
    //      database.
    static const std::string kNumSnapshots;

    //  "vidardb.oldest-snapshot-time" - returns number representing unix
    //      timestamp of oldest unreleased snapshot.
    static const std::string kOldestSnapshotTime;

    //  "vidardb.num-live-versions" - returns number of live versions. `Version`
    //      is an internal data structure. See version_set.h for details. More
    //      live versions often mean more SST files are held from being deleted,
    //      by iterators or unfinished compactions.
    static const std::string kNumLiveVersions;

    //  "vidardb.current-super-version-number" - returns number of current LSM
    ...
    //  "vidardb.estimate-live-data-size" - returns an estimate of the amount of
    //      live data in bytes.
    static const std::string kEstimateLiveDataSize;

    //  "vidardb.total-sst-files-size" - returns total size (bytes) of all SST
    //      files.
    //  WARNING: may slow down online queries if there are too many files.
    static const std::string kTotalSstFilesSize;

    //  "vidardb.base-level" - returns number of level to which L0 data will be
    //      compacted.
    static const std::string kBaseLevel;

    //  "vidardb.estimate-pending-compaction-bytes" - returns estimated total
    ...
    //  "vidardb.aggregated-table-properties" - returns a string representation
    //      of the aggregated table properties of the target column family.
    static const std::string kAggregatedTableProperties;

    //  "vidardb.aggregated-table-properties-at-level<N>", same as the previous
  ...
  // properties:
  //  "vidardb.num-immutable-mem-table"
  //  "vidardb.mem-table-flush-pending"
  //  "vidardb.compaction-pending"
  //  "vidardb.background-errors"
  //  "vidardb.cur-size-active-mem-table"
  //  "vidardb.cur-size-all-mem-tables"
  //  "vidardb.size-all-mem-tables"
  //  "vidardb.num-entries-active-mem-table"
  //  "vidardb.num-entries-imm-mem-tables"
  //  "vidardb.num-deletes-active-mem-table"
  //  "vidardb.num-deletes-imm-mem-tables"
  //  "vidardb.estimate-num-keys"
  //  "vidardb.estimate-table-readers-mem"
  //  "vidardb.is-file-deletions-enabled"
  //  "vidardb.num-snapshots"
  //  "vidardb.oldest-snapshot-time"
  //  "vidardb.num-live-versions"
  //  "vidardb.current-super-version-number"
  //  "vidardb.estimate-live-data-size"
  //  "vidardb.total-sst-files-size"
  //  "vidardb.base-level"
  //  "vidardb.estimate-pending-compaction-bytes"
  //  "vidardb.num-running-compactions"
  //  "vidardb.num-running-flushes"
...
#endif  // STORAGE_VIDARDB_INCLUDE_DB_H_
